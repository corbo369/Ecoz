...
contract BushBucksURI is Ownable {
    ...
    mapping (uint256 => string) buckIDToHash;
    ...
    function buckRandomizer(address user, uint256 buckID) external returns(uint256) {
        ...
        uint256 thisTrait;
        if (buckID <= 2250 && buckID > 1353) {
            uint256 value = 7;

            for (uint256 i = 0; i < traitCount; i++) {
                uint256 rand = random(user, buckID);

                thisTrait = traitGenerator(traitTypeCount[i], rand);
                buckIDToHash[buckID] = string(abi.encodePacked(buckIDToHash[buckID], Library.toString(thisTrait)));
            ...
            for (uint256 i = 0; i < babyTraitCount; i++) {
                uint256 rand = random(user, buckID);
                buckIDToHash[buckID] = string(abi.encodePacked(buckIDToHash[buckID], Library.toString(traitGenerator(babyTraitTypeCount[i], rand))));
            }
            return 0;
        }
        else {
    ...
    function random(address user, uint256 buckID) internal returns (uint256) {
        randI = randI + 1;
        
        if (randI >= 10) {
            randI = 0;
        }
        return uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, user, buckID, randI))) % 10000;
    }
    ...
    function bushBuckTokenURI(uint256 buckID, uint256 buckWeight) external view virtual returns(string memory) {
        require(msg.sender == address(ecozContract));

        if (buckID <= 2250) {
            string memory metadataString = '{"trait_type":"Species","value":"Bush Buck"},';
            ...
            if (buckID == 1351) {
                thisTrait = Leg[0];
                metadataString = string(abi.encodePacked(metadataString,'{"trait_type":"','1/1','","value":"','Patronous''"},','{"trait_type":"','Generation','","value":"','Genesis''"},','{"trait_type":"','Thrive Production','","value":"',Library.toString(buckWeight),'"}'));
                BGcolor = '4a5551';
            }

            else if (buckID == 1352) {
                thisTrait = Leg[1];
                metadataString = string(abi.encodePacked(metadataString,'{"trait_type":"','1/1','","value":"','Zombuck''"},','{"trait_type":"','Generation','","value":"','Genesis''"},','{"trait_type":"','Thrive Production','","value":"',Library.toString(buckWeight),'"}'));
                BGcolor = '98e2bb';
            }

            else if (buckID == 1353) {
                thisTrait = Leg[2];
                metadataString = string(abi.encodePacked(metadataString,'{"trait_type":"','1/1','","value":"','Jimbo''"},','{"trait_type":"','Generation','","value":"','Genesis''"},','{"trait_type":"','Thrive Production','","value":"',Library.toString(buckWeight),'"}'));
                BGcolor = '000000';
            }
            else {
                metadataString = string(abi.encodePacked(metadataString,'{"trait_type":"','Generation','","value":"','Genesis''"},','{"trait_type":"','Thrive Production','","value":"',Library.toString(buckWeight),'"},'));
            }
            
            //Get the metadata from the Traits using the hash
            if (buckID > 1353) {
                for (uint256 i = 0; i < traitCount; i++) {
                    thisTraitIndex = Library.parseInt(Library.substring(buckIDToHash[buckID], i, i + 1));
    ...
    function setBuckLegendary(string memory leg1, string memory leg2, string memory leg3, string memory unrevealed) public onlyOwner {
        Leg[0] = leg1;
        Leg[1] = leg2;
        Leg[2] = leg3;
        Leg[3] = unrevealed;
    ...
    function setBuckColorsLETTERS(string[87] memory colors, string[87] memory letters) public onlyOwner {
        Colors = colors;
        LETTERS = letters;
        return;
    }

    function addBuckTraitType(uint256 _traitTypeIndex, Trait[] memory traits) public onlyOwner {
    ...
    function addBabyBuckTraitType(uint256 _traitTypeIndex, Trait[] memory traits) public onlyOwner {
        for (uint256 i = 0; i < traits.length; i++) {
    ...
    function getHash(uint256 buckID) public view onlyOwner returns(string memory) {
        return(buckIDToHash[buckID]);
    }
    ...
    function setBuckEcoz(address ecozAddress) external onlyOwner {
